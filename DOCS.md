...
# Авторизация
Bitlle Network Api имеет 2 типа методов: публичные и приватные. Приватные методы выпоняются только авторизованными пользователями.
...
Получение json файла с зашифрованном приватным ключем
...
Следует учитывать комиссию (актульный размер которой можно получить при помощи метода [/api/v1/info/fee](#Получение-размера-комиссии)). Комиссия будет прибавляться к сумме перевода.
...
(необязательно) Актуализация баланса (для заяки на обмен)  
...
Получение размера актуальной коммисии  
